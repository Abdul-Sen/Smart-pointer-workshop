 ...
 w9::List<w9::Product> merge(const w9::List<w9::Description>& desc, const w9::List<w9::Price>& price) {
	...
	//w9::List<w9::Product>::iterator iter; //FOR PROF: Why am I not allowed to make an iterator here?
	 ...
	 for (int i = 0; i < desc.size(); i++)
	 {
		 for (int j = 0; j < price.size(); j++)
		 {
			 ...
			 {
				 // RAW PTR METHOD
				 //w9::Product* ptr = new w9::Product(desc[i].desc, price[j].price);
				 //priceList += ptr;
				 //delete ptr;
				 ...
				 ******************/
				 ...
				 //std::unique_ptr<w9::Product> ptr(new w9::Product(desc[i].desc, price[j].price));

				 // Apparently MakeUnique is for exception safety, it catches an exception throw 
				 //from a constructor and avoids having a dangling ptr in case our constructor fails

				 std::unique_ptr<w9::Product> ptr = std::make_unique<w9::Product>(desc[i].desc, price[j].price);
				 ptr->validate();
